...
async function getCalendarEvents(date: string): Promise<CalendarEvent[]> {
  ...
  if (!credentials) {
    console.log('No Google Calendar credentials configured - showing all slots');
    return [];
  }
  ...
  try {
    ...
    if (!key.private_key) {
      console.log('Invalid Google Calendar credentials - showing all slots');
      return [];
    }
    ...
    console.log(`Fetching calendar events for ${date}, timeMin: ${timeMin}, timeMax: ${timeMax}`);
      ...
      for (const event of response.data.items) {
        if (event.start && event.end) {
          ...
          if (start && end) {
            ...
            console.log(`Found event: ${event.summary}, start: ${start}, end: ${end}`);
          }
        }
      }
    }

    console.log(`Total ${events.length} calendar events found for ${date}`);
    return events;
  } catch (error) {
    console.error('Failed to fetch calendar events:', error);
    // エラーが発生した場合は空配列を返して、全ての時間枠を表示
    return [];
  }
}
...
function dateTimeToMinutes(dateTimeStr: string, targetDate: string): { start: number; end: number } | null {
  try {
    // 終日イベントの場合（日付のみ、時間なし）- "2024-01-01" format
    if (dateTimeStr.length === 10) {
      return null; // 終日イベントは別途処理
    }

    const dateTime = new Date(dateTimeStr);

    // 日本時間での時間を取得
    const hours = parseInt(dateTime.toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo', hour: '2-digit', hour12: false }));
    const minutes = parseInt(dateTime.toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo', minute: '2-digit' }));

    return { start: hours * 60 + minutes, end: 0 };
  } catch {
    return null;
  }
}
...
function isSlotOverlapping(
  ...
  for (const event of events) {
    // 終日イベントの場合はスキップ（終日イベントでは予約枠をブロックしない）
    // 終日イベントをブロックしたい場合はこの条件を変更
    ...
    try {
      ...
      // 日本時間での時間を取得
      const eventStartHours = parseInt(eventStartDate.toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo', hour: '2-digit', hour12: false }));
      const eventStartMinutes = parseInt(eventStartDate.toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo', minute: '2-digit' }));
      const eventStart = eventStartHours * 60 + eventStartMinutes;

      const eventEndHours = parseInt(eventEndDate.toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo', hour: '2-digit', hour12: false }));
      const eventEndMinutes = parseInt(eventEndDate.toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo', minute: '2-digit' }));
      const eventEnd = eventEndHours * 60 + eventEndMinutes;

      // 重複チェック: 時間枠と予定が重なっているか
      // 重なる条件: slotStart < eventEnd && slotEnd > eventStart
      if (slotStart < eventEnd && slotEnd > eventStart) {
        console.log(`Slot ${slotStart}-${slotEnd} overlaps with event ${event.summary} (${eventStart}-${eventEnd})`);
        return true;
      }
    } catch (e) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (date && typeof date === 'string') {
      console.log(`Processing slots request for date: ${date}`);
        ...
        response = DEFAULT_TIME_SLOTS
          .map((slot, index) => {
            ...
            if (slotStartMinutes < currentMinutes) {
              console.log(`Skipping past slot: ${slot.startTime} (${slotStartMinutes} < ${currentMinutes})`);
              return null;
            }
            ...
            const isOverlapping = isSlotOverlapping(slotStartMinutes, slotEndMinutes, calendarEvents, date);
            if (isOverlapping) {
              console.log(`Skipping overlapping slot: ${slot.startTime}`);
              return null;
            }
      ...
      console.log(`Returning ${response.length} available slots`);
